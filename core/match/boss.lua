...
function Boss:new(values)
    ...
    self.force = values.force or 100
...
function Boss:load()
    ...
    -- ANIM8
    local g = anim8.newGrid(64, 64, self.sprite.width, self.sprite.height)
    ...
    self.tween = flux.to(self, 4, { x = 400, y =  300})
...
function Boss:__getCenter()
    local x = (love.graphics.getWidth() / 2)
    local y = (love.graphics.getHeight() / 2)
    return x, y
end

function Boss:speech()
    -- death    
